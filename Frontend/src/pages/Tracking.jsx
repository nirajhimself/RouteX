...
export default function Tracking() {
  ...
  const loadStats = useCallback(async () => {
    try {
      ...
      const parked = list.filter(
        (v) => (v.status || "").toLowerCase() === "parked",
      ).length;
  ...
  const handleLocationUpdate = async () => {
    if (!updateForm.vehicle_id || !updateForm.lat || !updateForm.lng) {
  ...
  const STAT_ITEMS = [
    ...
    [
      "Avg Speed",
      statsLoading ? "…" : `${stats.avgSpeed} km/h`,
      "#818cf8",
      ClockIcon,
    ],
  ];
    ...
    <div className="space-y-5 animate-fade-in">
      <div className="page-header">
        <div>
          ...
          <p className="section-sub">
            // real-time fleet telemetry · polling every{" "}
      ...
      <div className="card p-5 max-w-lg">
        ...
        >
          ...
          <span className="ml-2 font-mono text-[9px] text-slate-600 font-normal">
            → POST /update-location
          </span>
        </h3>
        <div className="space-y-3">
          {[
            ["Vehicle ID", "vehicle_id", "text", "e.g. TRK-019"],
            ["Latitude", "lat", "number", "e.g. 19.076"],
            ["Longitude", "lng", "number", "e.g. 72.877"],
            ["Speed (km/h)", "speed", "number", "e.g. 67"],
            ...
            <div key={key} className="grid grid-cols-3 items-center gap-3">
              ...
              <input
                ...
                onChange={(e) =>
                  setUpdateForm((f) => ({ ...f, [key]: e.target.value }))
                }
          ...
          <div className="flex items-center gap-3 pt-2">
            ...
            {updateStatus === "success" && (
              <span className="badge badge-green">✓ Location updated</span>
            )}
            {updateStatus === "failed" && (
              <span className="badge badge-red">✗ Backend error</span>
            )}
            {updateStatus === "error" && (
              <span className="badge badge-red">All fields required</span>
            )}
          </div>
        </div>
      </div>
